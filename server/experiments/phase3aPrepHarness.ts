 ...
 * INDEPENDENT of `phase3EntryPoint.ts`'s `phase3a.v1`. The two are
 ...
 * step.
 *
 * Authority: this file is NOT in `phase3EntryPoint.ts`'s
 * `PHASE3_NEVER_AUTHORIZED_BY` list. Adding it there will be a separate
 * Phase 3a-prep-b PR that bumps the entry-point schema version
 * (`phase3a.v1 -> phase3a.v2`) and updates every dependent assertion.
 * Until then, importing this file confers no authority — it is a
 * declarative schema anchor only.
 * ─────────────────────────────────────────────────────────────────────────────
 */
