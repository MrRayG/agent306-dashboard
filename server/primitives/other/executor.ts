// ---------------------------------------------------------------------------
// 306 — OTHER PRIMITIVE EXECUTOR (scaffold, dry-run by default)
//
// Third concrete primitive executor wired onto the registry scaffolding
// landed by PR #423 (commit 199be6d), following the proven synthesis
// executor pattern from PR #425 (commit b123b7f) and the artifact
// executor pattern from PR #426 (commit 70b6cd6). The registry seam
// exists but is never dispatched from: `actionTranslator.translateAction`
...
//
// The `other` family is the catch-all bucket for missing-primitive
// classifications that did not match any of the more specific families
// (artifact, ratio, ttl, gate, archive, spectrum, synthesis, rewrite,
// verification, verification_scaffold). It exists so the classifier in
// `actionTranslator.classifyMissingPrimitiveFamily` is total — every
// unparseable action gets a family label. The executor below mirrors
// that catch-all posture: it emits structured telemetry for any action
// that reaches it, without claiming to know how to handle it.
//
// What this module DOES today
// ---------------------------
//   - Exposes the `other::scaffold` Primitive descriptor.
...
//     for telemetry only.
//   - Honours `PRIMITIVE_OTHER_EXECUTOR_DRY_RUN` (default `true` when
...
//     perform real work until a real engine is injected.
//
// What this module DOES NOT do today
// ----------------------------------
//   - Run any production engine, briefing pipeline, thread/post
//     publisher, or LLM call. Real engine wiring is deferred to a
//     follow-up PR once dry-run telemetry has been observed stable
...
//     Registration only happens through `registerOtherPrimitive()`,
...
/**
 * Env flag controlling whether the other executor should be registered
 ...
 */
export const PRIMITIVE_OTHER_EXECUTOR_ENABLED_ENV =
 ...
 */
export const PRIMITIVE_OTHER_EXECUTOR_DRY_RUN_ENV =
  "PRIMITIVE_OTHER_EXECUTOR_DRY_RUN";

/** Stable family/id key. Mirrors the `family::id` convention used by registry. */
export const OTHER_PRIMITIVE_ID = "scaffold";
 ...
 * PR #419 / PR #423 / PR #425 / PR #426 convention).
 */
export function isOtherExecutorEnabled(): boolean {
  return process.env[PRIMITIVE_OTHER_EXECUTOR_ENABLED_ENV] === "true";
}
 ...
 */
export function isOtherExecutorDryRun(): boolean {
  return process.env[PRIMITIVE_OTHER_EXECUTOR_DRY_RUN_ENV] !== "false";
}
 ...
 */
export const otherExecutor: PrimitiveExecutor = async (
  ctx: PrimitiveExecutionContext,
): Promise<PrimitiveExecutionResult> => {
  const dryRun = isOtherExecutorDryRun();
  ...
  if (!dryRun) {
    ...
    return {
      ...
      sideEffects: [],
      reason:
        "other-executor: non-dry-run requested but no production engine is wired; refusing",
    };
  }
  ...
  return {
    ...
    sideEffects: [
      ...
      `[dry-run] would-handle-other from action="${ctx.actionText.slice(0, 80)}"`,
    ],
  };
};
 ...
 */
export const OTHER_PRIMITIVE: Primitive = {
  family: "other",
  id: OTHER_PRIMITIVE_ID,
  description:
    "Dry-run other (catch-all) primitive scaffold (telemetry only; no production engine wired).",
  execute: otherExecutor,
};
