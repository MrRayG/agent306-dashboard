/**
 * Tests for server/articleReviseLoop.ts (issue 2)
 *
 * The revise loop sits between the writer and the verifier. We drive the
 * loop with a stub rewriter so the test stays hermetic — no LLM calls.
 *
 * Run: npx tsx --test server/__tests__/articleReviseLoop.test.ts
 */
...
describe("reviseUntilClean — basic loop", () => {
