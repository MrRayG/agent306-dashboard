/**
 * blogReviseLoop — judge-outage handling.
 *
 * Mirrors articleReviseLoop.judgeOutage.test.ts. The blog revise loop must
 * not retry indefinitely when the verifier judge is down. There is a
 * JUDGE_OUTAGE_RETRY_CAP (=2) so after two consecutive judge-outage
 * verdicts the loop stops, leaves the verdict as HARD_FAIL (so blogEngine
 * quarantines as the existing fallback), and records "held_for_review"
 * in revisionHistory.
 *
 * Run: npx tsx --test server/__tests__/blogReviseLoop.judgeOutage.test.ts
 */
...
describe("reviseBlogUntilClean — judge outage retry cap", () => {
